...
FROM 
    `site`.`user` u
LEFT JOIN 
    `site`.`profile` p ON u.iduser = p.iduser
LEFT JOIN 
    `site`.`user_role` ur ON u.iduser = ur.iduser
LEFT JOIN 
    `site`.`role` r ON ur.idrole = r.idrole;
...
VALUES 
  (1, 'The admin of a site', 'Administrator'),
...
VALUES 
  (1, 1, 'Kenji', 'Vue', 'M', '2000-01-31', 'KenjiMVue.jpg', '#431499', '<p>I am a life long anime fan ever since I saw my first series "Macross" all the way back in 1990. Yep, I am really old, [...]</p>'),
  (2, 7, 'Bruce', 'Wayne', NULL, '1990-12-25', 'bruce.jpg', '#0000ff', 'Batman is the superhero protector of Gotham City, a man dressed like a bat who fights against evil and strikes terror into [...]'),
  (3, 8, 'Joe', 'Joestar', NULL, '1980-05-03', 'joe.png', '#00ff00', 'Joseph is a natural-born Ripple user and eventual Stand user, wielding the psychic photographic Stand, Hermit Purple. Joseph [...]');
